...
public class WalkerRunnableTest {
    static class Callback implements RunnableCallback<IPolyline> {
        @Override
    ...
    public void testRun() {
        ...
        try (MockedConstruction<Walker> walkerMockedConstruction = Mockito.mockConstruction(Walker.class, (mock, context) -> when(mock.walk(Mockito.any(), Mockito.any())).thenReturn(polyline))) {
            WalkerRunnable runnable = new WalkerRunnable(cellMock, handlerMock, callback);
            runnable.run();
